...
interface ProductSimpleAdditionalProps {
  additional: OrderProductAdditional[];
}

export default function ProductSimpleAdditional({ additional }: ProductSimpleAdditionalProps) {
      ...
      <List className={classes.list}>
        {additional.map(item => (
          <ListItem button className={item.selected ? classes.selected : classes.listItem} key={item.id}>
            ...
            <ProductViewAmountControl selectedAmount={item.amount} />
          </ListItem>
        ))}
      </List>
    </Fragment>
